...
const PoolsDetail = ({
  ...
  const VestingCompletionist = () => (
    <span className="inline-block">released</span>
  );
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  const idos = idoInfo.filter((idoinfo) => {
    return idoinfo.id == poolId;
  })[0];
  ...
  async function status() {
    ...
    if (blocknumber < idos.startblock) {
      ...
      counttime.current.innerText = "End in:";
    } else if (blocknumber > idos.endblock) {
      counttime.current.innerText = "Ended";
    }
  }
  ...
  async function Vestingstatus() {
    ...
    } else if (blocknumber > idos.endblock && blocknumber < idos.vestingblock) {
      vestingcounttime.current.innerText = "";
    } else if (blocknumber > idos.vestingblock) {
      vestingcounttime.current.innerText = " " + "";
    }
  }
  ...
  async function Partrate() {
    console.log(idos.contract);
    let instance = new web3.eth.Contract(bullAbi, idos.contract).methods;
    instance
      .idoInfo()
      .call()
      .then((response) => {
        setpartrate(100 / (response[2] / response[3]));
      });
    console.log(partrate);
    return null;
  }
  const [ushare, sushare] = useState(0);
  async function userTokenShare() {
    console.log(idos.contract);

    console.log(ushare + "its user share");
    return null;
  }
  ...
  useEffect(() => {
    let instance = new web3.eth.Contract(bullAbi, address).methods;
    if (user == true) {
    ...
    new Web3("https://data-seed-prebsc-1-s1.binance.org:8545").eth
      ...
      .then((value) => {
        ...
        console.log(blockNumberss + "this is block number");
      });
    instance
      .idoInfo()
      .call()
  ...
  Vestingstatus();
  //idotimeing();
  let address = idos.idocontract;
  let user = active;
  let useraddr = { account }.account;
  ...
  console.log(
    "idos vesting block is:",
    Date.now() + (idos.vestingblock - blockNumberss) * 3 * 1000
  );
  ...
  return (
    <div className="relative dark:bg-darkMode-600">
      ...
      <div className="container mx-auto">
        ...
        />
        {/* <Userinfo
          address={idos.idocontract}
          user={active}
          useraddr={{ account }.account}
        /> */}
      </div>
      <section className="bg-gray-100 dark:bg-darkMode-600">
        <div className={"relative container mx-auto px-4 lg:px-14 mt-6"}>
            ...
            >
              <div className="p-4 flex flex-col items-center w-full">
                ...
                <PhoneIdoHeader
                  ...
                  darkMode={darkMode}
                >
                  {partrate}% <span className="font-medium text-sm">sold</span>
                </PhoneIdoHeader>
              ...
              <div className="p-4 flex flex-col items-center w-full border-t border-gray-200 dark:border-gray-700 text-sm">
                <SummaryDetail>
                  ...
                  <div className="flex items-center space-x-1">
                    <AddressInput
                      ...
                      value={
                        BSCAddress.substr(0, 4) +
                        "...." +
                        BSCAddress.substr(38, 42)
                      }
                    />
                ...
                <SummaryDetail>
                  ...
                  <div className="flex items-center text-gray-800 dark:text-white">
                    <span>
                      {idos.offeringallocation / idos.rasingallocation} $
                    </span>
                  </div>
                </SummaryDetail>
                <SummaryDetail>
                  <span
                    ref={counttime}
                    className="inline-block text-gray-800 dark:text-white counttime"
                  >
                    Start in:
                  </span>
                  <div className="flex items-center text-gray-800 dark:text-white">
                    <span>
                      <Countdown
                        date={
                          Date.now() +
                          (idos.startblock - blockNumberss) * 3 * 1000
                        }
                      >
                        <Completionist />
                      </Countdown>
                ...
                <SummaryDetail>
                  ...
                  <div
                    className="flex items-center text-green-600 dark:text-green-500 font-bold underline cursor-pointer"
                    onClick={Claim}
                  >
                    Claim
                  </div>
                  ...
                  <li className="grid grid-cols-2 mt-1.5 bg-gray-100 text-gray-900 dark:text-white dark:bg-darkMode-600 px-3 py-3 rounded-md leading-none">
                    ...
                    <span>
                      <Countdown
                        date={
                          Date.now() +
                          (idos.vestingblock - blockNumberss) * 3 * 1000
                        }
                      ></Countdown>
                      <span
                        ref={vestingcounttime}
              ...
              <div className="bg-white border-2 border-gray-200 dark:border-opacity-0 dark:bg-darkMode-800 px-4 py-5 mb-6 rounded-lg">
                ...
                <ul>
                  ...
                  <li className="grid grid-cols-2 mt-1.5 bg-gray-100 text-gray-900 dark:text-white dark:bg-darkMode-600 px-3 py-3 rounded-md leading-none">
                    ...
                    <span className="text-sm">{ushare}</span>
                  </li>

                  <li className="grid grid-cols-2 mt-1.5 bg-gray-100 text-gray-900 dark:text-white dark:bg-darkMode-600 px-3 py-3 rounded-md leading-none">
                    <span>Whitelist:</span>
                    <span>{ui[0] ? "Yes" : "No"}</span>
                  </li>
                </ul>
              </div>
          ...
          <button
            onClick={async () => {
              await console.log(
                await new library.eth.Contract(
                  bullAbi,
                  idos.idocontract
                ).methods
              ...
              try {
                await console.log(
                  await new library.eth.Contract(
                    ...
                    .call()
                    .then((response) => {
                      alert(response.participationAmount);
                    })
                );
              } finally {
              }
